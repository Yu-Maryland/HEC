    ...
    IterData: IterationData<L, N>,
{
    ...
    /// Sets the runner time limit using seconds as f32. Default: 5 seconds
    pub fn with_time_limit_secs(self, time_limit: f32) -> Self {
        Self { time_limit: Duration::from_secs_f32(time_limit), ..self }
    }
    ...
    N: Analysis<L>,
{
    ...
    /// Plan whether this rule is allowed to search this round and the matching threshold; None = banned this round.
    /// Default: no limit (maintains compatibility with SimpleScheduler)
    fn plan_for_rule(&self, _iteration: usize, _rewrite: &Rewrite<L, N>) -> Option<usize> {
        Some(usize::MAX)
    }

    /// After parallel search ends, write back the total matches and the threshold at that time to the statistics/ban state.
    /// Default: do nothing (maintains compatibility with SimpleScheduler)
    fn on_searched(
        &mut self,
        _iteration: usize,
        _rewrite: &Rewrite<L, N>,
        _total_matches: usize,
        _planned_threshold: usize,
    ) { /* no-op */ }
}
    ...
    N: Analysis<L>,
{
    ...
    fn plan_for_rule(&self, iteration: usize, rewrite: &Rewrite<L, N>) -> Option<usize> {
        if let Some(s) = self.stats.get(&rewrite.name) {
            if iteration < s.banned_until { return None; }
            let threshold = s.match_limit.checked_shl(s.times_banned as u32).unwrap();
            Some(threshold)
        } else {
            Some(self.default_match_limit)
        }
    }
    ...
    fn on_searched(
        ...
        rewrite: &Rewrite<L, N>,
        total_matches: usize,
        planned_threshold: usize,
    ) {
        let stats = self.rule_stats(rewrite.name);
        if iteration < stats.banned_until { return; }

        if total_matches > planned_threshold {
            let ban_len = stats.ban_length << stats.times_banned;
            stats.times_banned += 1;
            stats.banned_until = iteration + ban_len;
            info!(
                "Banning {} ({}-{}) for {} iters: {} < {}",
                rewrite.name, stats.times_applied, stats.times_banned,
                ban_len, planned_threshold, total_matches
            );
        } else {
            stats.times_applied += 1;
        }
    ...
    //             // m.ast: Option<Cow<PatternAst<L>>> -> Option<&PatternAst<L>>
    //             m.ast.as_ref().map(|cow| cow.as_ref())
    //         } else {
    //             None
    //         };
    //         for subst in &m.substs {
    //             let n0 = egraph.total_size();
    //             let e0 = egraph.number_of_classes();

    //             let out = rewrite.applier.apply_one(
    //                 egraph,
    //                 m.eclass,
    //                 subst,
    //                 ast,
    ...
    //             let n1 = egraph.total_size();
    //             let e1 = egraph.number_of_classes();

    //             if !out.is_empty() || n1 != n0 || e1 != e0 {
    //                 changed += 1;
    //             }
    //         }
    //     }
